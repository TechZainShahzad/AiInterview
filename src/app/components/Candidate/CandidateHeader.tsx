...
const CandidateHeader: React.FC<CandidateHeaderProps> = ({ activeTab, onTabSwitch, userName }) => {
  ...
  return (
    <header className="bg-[var(--background)] py-4">
      <div className="container mx-auto flex justify-between items-center px-4">
        <Link href="/candidate" className="text-2xl md:text-2xl lg:text-3xl text-[var(--foreground)]">
        Alnterview Hub
        </Link>
        <div className="flex space-x-4 items-center">
          ...
          <button
            onClick={() => onTabSwitch('join-interview')}
            className={`bg-[var(--background)] text-[var(--foreground)] hover:bg-[var(--foreground)] hover:text-white font-medium px-4 py-2 rounded-md transition-colors duration-300 ${
              activeTab === 'join-interview' ? 'bg-foreground text-white' : ''
            }`}
          >
            Join Interview
          </button>
          
          {/* Circle with initials as profile button */}
